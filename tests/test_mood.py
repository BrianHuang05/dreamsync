...
class MoodDropDetectionTests(unittest.TestCase):
    ...
    def test_drop_on_rms_spike_after_dip(self) -> None:
        mc = self._make()
        # Dip: RMS drops below drop_rms_dip (0.10)
        mc.update(ema_rms=0.06, stability=0.03, bpm=120.0, t=0.0)
        # Spike: RMS jumps by >= drop_rms_spike (0.15) within drop_window (0.5s)
        mood = mc.update(ema_rms=0.25, stability=0.03, bpm=120.0, t=0.3)
    ...
    def test_no_drop_without_dip(self) -> None:
        ...
        mc = self._make()
        mc.update(ema_rms=0.20, stability=0.03, bpm=120.0, t=0.0)
        mood = mc.update(ema_rms=0.40, stability=0.03, bpm=120.0, t=0.3)
    ...
    def test_no_drop_if_spike_too_late(self) -> None:
        ...
        mood = mc.update(ema_rms=0.25, stability=0.03, bpm=120.0, t=1.0)
        self.assertNotEqual(mood, Mood.DROP)
    ...
    def test_drop_auto_expires(self) -> None:
        ...
        # Trigger DROP
        mc.update(ema_rms=0.06, stability=0.03, bpm=120.0, t=0.0)
        mc.update(ema_rms=0.25, stability=0.03, bpm=120.0, t=0.3)
        self.assertEqual(mc.mood, Mood.DROP)
        # Still DROP within duration
        mood = mc.update(ema_rms=0.25, stability=0.03, bpm=120.0, t=1.5)
        self.assertEqual(mood, Mood.DROP)
        # After drop_duration, transitions out
        mood = mc.update(ema_rms=0.25, stability=0.03, bpm=120.0, t=2.5)
        self.assertNotEqual(mood, Mood.DROP)
    ...
    def test_drop_cooldown_prevents_rapid_refire(self) -> None:
        ...
        self.assertEqual(mc.mood, Mood.DROP)
        # Expire first DROP
        mc.update(ema_rms=0.25, stability=0.03, bpm=120.0, t=2.0)
        self.assertNotEqual(mc.mood, Mood.DROP)
        # Try second DROP within cooldown (10s)
        mc.update(ema_rms=0.06, stability=0.03, bpm=120.0, t=3.0)
        mood = mc.update(ema_rms=0.25, stability=0.03, bpm=120.0, t=3.3)
        self.assertNotEqual(mood, Mood.DROP)
...
class MoodHysteresisTests(unittest.TestCase):
    ...
    def test_chill_to_groove_requires_exit_threshold(self) -> None:
        ...
        mc = self._make()
        # Start in CHILL
        mc.update(ema_rms=0.05, stability=0.04, bpm=120.0, t=0.0)
        self.assertEqual(mc.mood, Mood.CHILL)
        # RMS = 0.11: above chill_rms_ceiling (0.10) but below chill_rms_exit (0.13)
        mood = mc.update(ema_rms=0.11, stability=0.04, bpm=120.0, t=1.0)
        self.assertEqual(mood, Mood.CHILL, "Should stay CHILL due to hysteresis")
        # RMS = 0.15: above chill_rms_exit (0.13) → transitions
        mood = mc.update(ema_rms=0.15, stability=0.04, bpm=120.0, t=2.0)
        self.assertEqual(mood, Mood.GROOVE)
    ...
    def test_groove_to_hype_requires_exit_threshold(self) -> None:
        ...
        mc = self._make()
        # Enter GROOVE
        mc.update(ema_rms=0.15, stability=0.04, bpm=120.0, t=0.0)
        self.assertEqual(mc.mood, Mood.GROOVE)
        # RMS = 0.25: above groove_rms_ceiling (0.24) but below groove_rms_exit_high (0.27)
        mood = mc.update(ema_rms=0.25, stability=0.04, bpm=120.0, t=1.0)
        self.assertEqual(mood, Mood.GROOVE, "Should stay GROOVE due to hysteresis")
        # RMS = 0.30: above groove_rms_exit_high (0.27) → transitions
        mood = mc.update(ema_rms=0.30, stability=0.04, bpm=120.0, t=2.0)
        self.assertEqual(mood, Mood.HYPE)
    ...
    def test_hype_to_groove_requires_exit_threshold(self) -> None:
        ...
        mc = self._make()
        # Enter HYPE
        mc.update(ema_rms=0.30, stability=0.04, bpm=128.0, t=0.0)
        self.assertEqual(mc.mood, Mood.HYPE)
        # RMS = 0.22: below groove_rms_ceiling (0.24) but above hype_rms_exit (0.20)
        mood = mc.update(ema_rms=0.22, stability=0.04, bpm=128.0, t=1.0)
        self.assertEqual(mood, Mood.HYPE, "Should stay HYPE due to hysteresis")
        # RMS = 0.18: below hype_rms_exit (0.20) → transitions
        mood = mc.update(ema_rms=0.18, stability=0.04, bpm=120.0, t=2.0)
...
class MoodDwellTimeTests(unittest.TestCase):
    ...
    def test_dwell_prevents_early_switch(self) -> None:
        ...
        mood = mc.update(ema_rms=0.35, stability=0.03, bpm=128.0, t=2.0)
        ...
        mood = mc.update(ema_rms=0.35, stability=0.03, bpm=128.0, t=5.0)
        self.assertEqual(mood, Mood.HYPE)
    ...
    def test_drop_ignores_dwell(self) -> None:
        ...
        mc.update(ema_rms=0.06, stability=0.03, bpm=120.0, t=0.8)
        mood = mc.update(ema_rms=0.25, stability=0.03, bpm=120.0, t=1.0)
        self.assertEqual(mood, Mood.DROP)
...
class MoodIntegrationTests(unittest.TestCase):
    ...
    def test_song_simulation(self) -> None:
        ...
        mood = mc.update(ema_rms=0.04, stability=0.15, bpm=0.0, t=0.0)
        self.assertEqual(mood, Mood.CHILL)

        # --- Verse: moderate energy, stable beat ---
        mood = mc.update(ema_rms=0.16, stability=0.04, bpm=110.0, t=5.0)
        self.assertEqual(mood, Mood.GROOVE)

        # --- Chorus: high energy ---
        mood = mc.update(ema_rms=0.30, stability=0.03, bpm=128.0, t=10.0)
        self.assertEqual(mood, Mood.HYPE)

        # --- Drop: energy dips then spikes ---
        mc.update(ema_rms=0.05, stability=0.03, bpm=128.0, t=15.0)
        mood = mc.update(ema_rms=0.35, stability=0.03, bpm=128.0, t=15.3)
        self.assertEqual(mood, Mood.DROP)

        # --- DROP auto-expires into HYPE ---
        mood = mc.update(ema_rms=0.30, stability=0.03, bpm=128.0, t=18.0)
        self.assertIn(mood, (Mood.HYPE, Mood.GROOVE))

        # --- Outro: energy fades ---
        mood = mc.update(ema_rms=0.04, stability=0.15, bpm=0.0, t=25.0)
        self.assertEqual(mood, Mood.CHILL)
