...
class EffectCyclerSelectionTests(unittest.TestCase):
    ...
    def test_chill_selects_from_chill_pool(self) -> None:
        ...
        preset = cycler.update(Mood.CHILL, t=0.0, beat=False, bpm=90.0, rms=0.05)
    ...
    def test_groove_selects_from_groove_pool(self) -> None:
        cycler = EffectCycler(seed=42)
        preset = cycler.update(Mood.GROOVE, t=0.0, beat=False, bpm=120.0, rms=0.15)
    ...
    def test_hype_selects_from_hype_pool(self) -> None:
        cycler = EffectCycler(seed=42)
        preset = cycler.update(Mood.HYPE, t=0.0, beat=False, bpm=128.0, rms=0.30)
    ...
    def test_drop_always_returns_drop_blast(self) -> None:
        cycler = EffectCycler(seed=42)
        preset = cycler.update(Mood.DROP, t=0.0, beat=True, bpm=128.0, rms=0.35)
...
class EffectCyclerTimingTests(unittest.TestCase):
    ...
    def test_cycling_after_interval(self) -> None:
        ...
        # Initial selection
        p1 = cycler.update(Mood.GROOVE, t=0.0, beat=False, bpm=120.0, rms=0.15)
        # Before interval: same effect
        p2 = cycler.update(Mood.GROOVE, t=5.0, beat=False, bpm=120.0, rms=0.15)
        self.assertEqual(p1.name, p2.name)
        # After interval: different effect (pool has >1 effects, so exclude works)
        p3 = cycler.update(Mood.GROOVE, t=11.0, beat=False, bpm=120.0, rms=0.15)
    ...
    def test_no_cycle_within_interval(self) -> None:
        ...
        p1 = cycler.update(Mood.CHILL, t=0.0, beat=False, bpm=90.0, rms=0.05)
        # Multiple updates within interval should return the same effect
        for t in [2.0, 4.0, 8.0, 12.0, 15.0]:
            p = cycler.update(Mood.CHILL, t=t, beat=False, bpm=90.0, rms=0.05)
...
class EffectCyclerMoodChangeTests(unittest.TestCase):
    ...
    def test_mood_change_triggers_new_effect(self) -> None:
        ...
        p2 = cycler.update(Mood.GROOVE, t=1.0, beat=False, bpm=120.0, rms=0.15)
        groove_names = {name for name, _ in MOOD_EFFECTS[Mood.GROOVE]}
    ...
    def test_mood_change_resets_cycle_timer(self) -> None:
        config = EffectCyclerConfig(cycle_interval=10.0)
        cycler = EffectCycler(config=config, seed=42)
        cycler.update(Mood.CHILL, t=0.0, beat=False, bpm=90.0, rms=0.05)
        # Switch mood at t=5
        p2 = cycler.update(Mood.GROOVE, t=5.0, beat=False, bpm=120.0, rms=0.15)
        # At t=12 (7s into GROOVE, < cycle_interval) should still be same
        p3 = cycler.update(Mood.GROOVE, t=12.0, beat=False, bpm=120.0, rms=0.15)
        self.assertEqual(p2.name, p3.name)
        # At t=16 (11s into GROOVE, > cycle_interval) should cycle
        p4 = cycler.update(Mood.GROOVE, t=16.0, beat=False, bpm=120.0, rms=0.15)
...
class EffectCyclerDropTests(unittest.TestCase):
    ...
    def test_drop_always_returns_drop_blast(self) -> None:
        cycler = EffectCycler(seed=42)
        # Start in GROOVE
        cycler.update(Mood.GROOVE, t=0.0, beat=False, bpm=120.0, rms=0.15)
        # Enter DROP
        preset = cycler.update(Mood.DROP, t=1.0, beat=True, bpm=128.0, rms=0.35)
        self.assertEqual(preset.name, "drop_blast")

    def test_drop_stays_drop_blast_while_in_drop(self) -> None:
        cycler = EffectCycler(seed=42)
        cycler.update(Mood.DROP, t=0.0, beat=True, bpm=128.0, rms=0.35)
        # Multiple DROP updates
        for t in [0.5, 1.0, 1.5]:
            preset = cycler.update(Mood.DROP, t=t, beat=True, bpm=128.0, rms=0.35)
            self.assertEqual(preset.name, "drop_blast")
    ...
    def test_leaving_drop_picks_from_new_mood(self) -> None:
        ...
        preset = cycler.update(Mood.HYPE, t=3.0, beat=False, bpm=128.0, rms=0.30)
        hype_names = {name for name, _ in MOOD_EFFECTS[Mood.HYPE]}
        self.assertIn(preset.name, hype_names)
    ...
    def test_leaving_drop_into_groove(self) -> None:
        cycler = EffectCycler(seed=42)
        cycler.update(Mood.DROP, t=0.0, beat=True, bpm=128.0, rms=0.35)
        preset = cycler.update(Mood.GROOVE, t=3.0, beat=False, bpm=120.0, rms=0.15)
        groove_names = {name for name, _ in MOOD_EFFECTS[Mood.GROOVE]}
        self.assertIn(preset.name, groove_names)
...
class EffectCyclerPaletteTests(unittest.TestCase):
    ...
    def test_palette_from_mood_eligible_set(self) -> None:
        cycler = EffectCycler(seed=42)
        for mood in Mood:
            preset = cycler.update(mood, t=0.0, beat=False, bpm=120.0, rms=0.15)
    ...
    def test_palette_changes_on_cycle(self) -> None:
        ...
        for seed in range(50):
            ...
            p1 = cycler.update(Mood.GROOVE, t=0.0, beat=False, bpm=120.0, rms=0.15)
            p2 = cycler.update(Mood.GROOVE, t=6.0, beat=False, bpm=120.0, rms=0.15)
...
class EffectCyclerIntegrationTests(unittest.TestCase):
    ...
    def test_song_simulation(self) -> None:
        ...
        # --- CHILL intro ---
        p = cycler.update(Mood.CHILL, t=0.0, beat=False, bpm=0.0, rms=0.04)
        self.assertIn(p.name, chill_names)
        first_chill = p.name

        # Still CHILL at t=5
        p = cycler.update(Mood.CHILL, t=5.0, beat=False, bpm=0.0, rms=0.04)
        self.assertEqual(p.name, first_chill)

        # --- GROOVE verse at t=8 ---
        p = cycler.update(Mood.GROOVE, t=8.0, beat=True, bpm=120.0, rms=0.15)
        self.assertIn(p.name, groove_names)

        # --- HYPE chorus at t=15 ---
        p = cycler.update(Mood.HYPE, t=15.0, beat=True, bpm=128.0, rms=0.30)
        self.assertIn(p.name, hype_names)

        # --- DROP at t=20 ---
        p = cycler.update(Mood.DROP, t=20.0, beat=True, bpm=128.0, rms=0.35)
        self.assertEqual(p.name, "drop_blast")

        # Still DROP at t=21
        p = cycler.update(Mood.DROP, t=21.0, beat=True, bpm=128.0, rms=0.35)
        self.assertEqual(p.name, "drop_blast")

        # --- Back to HYPE at t=23 ---
        p = cycler.update(Mood.HYPE, t=23.0, beat=True, bpm=128.0, rms=0.30)
        self.assertIn(p.name, hype_names)

        # --- CHILL outro at t=35 ---
        p = cycler.update(Mood.CHILL, t=35.0, beat=False, bpm=0.0, rms=0.04)
        self.assertIn(p.name, chill_names)
