...
class SpotifyLearnedLiveSession:
    ...
    def _on_track_changed(self, new_track, old_track) -> None:
        self._invalidate_active_capture("track_changed" if old_track is not None else "")
        ...
        with self._lock:
            ...
            self._snapshot = replace(
                ...
                active_learning_state="capturing" if learning else "not_learning",
                active_learning_reason=(
                    "" if learning else "paused"
                    if playback is not None and not getattr(playback, "is_playing", False)
                    else "started_mid_track" if progress > 2 else "disabled"
                ),
                learning_state="capturing" if learning else "not_learning",
                ...
                cache_misses=self._snapshot.cache_misses + 1,
            )
